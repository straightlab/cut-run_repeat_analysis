...
def run():
    ...
    if '-binarize' in sys.argv:
        BinThreshold = float(sys.argv[sys.argv.index('-binarize')+1])
    ...
    doFFMP = False
    if '-fullFragmentMidPoint' in sys.argv:
        doFFMP = True
        print('will use the midpoint of the fragment')
    ...
    if not doRPM and '-mappabilityNormalize' in sys.argv:
        ...
        for chr in list(MappabilityRegionDict.keys()):
            for (left,right) in list(MappabilityRegionDict[chr].keys()):
                TotalScore = 0.0
                for j in range(left,right):
...
#                print chrStats
        ...
        if doUniqueBAM:
            ...
            TotalReads = UniqueReads + len(list(MultiplicityDict.keys()))
...
#        print 'found', len(ERMTOCDict.keys()), 'multimappers'
        Excluded = 0
        for ID in list(ERMTOCDict.keys()):
            i+=1
            if i % 5000000 == 0:
            ...
            for chr in list(ERMTOCDict[ID].keys()):
                if chr not in list(chromInfoDict.keys()):
                    ToBeExcluded = True
                if chr in list(chromInfoDict.keys()):
        ...
        print('retained', len(list(ERMTOCDict.keys())), 'multimappers')
            ...
            for alignedread in samfile.fetch(chr, left, right):
                ...
                if doFFMP:
                    if alignedread.is_paired:
                        ...
                        JJ = int((matepos + pos + len(alignedread.query))/2.0)
                        if JJ >= left and JJ <= right:
                            pass
                        else:
                            continue
                    else:
